...
import { StoryEvent } from "../lib/StoryTypes";
...
async function testBasicFiltering() {
  const events: StoryEvent[] = [
    ...
    createStoryEvent("bill", "player", "I'm doing well, thanks for asking!", 1001),
...
async function testAndConditions() {
  const events: StoryEvent[] = [
    createStoryEvent("player", "bill", "Are you ready?", 2000, "p", "0.0", ["question"]),
    createStoryEvent("bill", "player", "Yes, I'm ready!", 2001, "p", "0.1", ["response"]),
    createStoryEvent("player", "bill", "Great!", 2002, "p", "0.2", ["exclamation"]),
    createStoryEvent("bill", "player", "Let's go then.", 2003, "p", "0.3", ["action"]),
    createStoryEvent("player", "narrator", "I follow Bill.", 2004, "action", "0.4", ["action"]),
    createStoryEvent("narrator", "player", "You both head towards the door.", 2005, "p", "0.5", ["narrative"]),
  ];

  const playerToBillQuestions = events.filter(sift({
    $and: [
      { from: "player" },
      { to: "bill" },
      { tags: { $in: ["question"] } }
  ...
  const billResponses = events.filter(sift({
    $and: [
      { from: "bill" },
      { to: "player" },
...
async function testOrConditions() {
  ...
  const playerOrBillEvents = events.filter(sift({
    $or: [
      { from: "player" },
      { from: "bill" }
    ]
  ...
  const narratorOrSystemEvents = events.filter(sift({
    $or: [
      { from: "narrator" },
      { from: "system" }
    ]
  ...
  const toBillOrNarrator = events.filter(sift({
    $or: [
      { to: "bill" },
      { to: "narrator" }
    ]
...
async function testComplexAndOrConditions() {
  const events: StoryEvent[] = [
    createStoryEvent("player", "bill", "What's the plan?", 4000, "p", "0.0", ["question"]),
    createStoryEvent("bill", "player", "We need to find the key.", 4001, "p", "0.1", ["plan"]),
    createStoryEvent("player", "narrator", "I nod in agreement.", 4002, "action", "0.2", ["action"]),
    createStoryEvent("narrator", "player", "Bill points to the locked door.", 4003, "p", "0.3", ["description"]),
    createStoryEvent("bill", "narrator", "The key should be around here somewhere.", 4004, "p", "0.4", ["plan"]),
    createStoryEvent("system", "all", "You hear footsteps approaching.", 4005, "p", "0.5", ["audio", "warning"]),
    createStoryEvent("player", "bill", "Someone's coming!", 4006, "p", "0.6", ["warning", "exclamation"]),
    createStoryEvent("bill", "player", "Quick, hide!", 4007, "p", "0.7", ["urgent", "action"]),
  ];

  const conversationEvents = events.filter(sift({
    $and: [
      {
        $or: [
          { from: "player", to: "bill" },
  ...
  const urgentOrWarningEvents = events.filter(sift({
    $or: [
      {
        $and: [
          { tags: { $in: ["warning"] } },
  ...
  const playerToBillOrNarratorEvents = events.filter(sift({
    $and: [
      { from: "player" },
      {
        $or: [
...
async function testDialogBetweenBillAndPlayer() {
  ...
  const billPlayerDialog = events.filter(sift({
    $or: [
      { from: "player", to: "bill" },
      { from: "bill", to: "player" }
    ]
  ...
  const nonDialogEvents = events.filter(sift({
    $and: [
      {
        $not: {
          $or: [
...
async function testTimeRangeFiltering() {
  ...
  const earlyConversation = events.filter(sift({
    $and: [
      { time: { $lte: 2000 } },
      {
        $or: [
  ...
  const laterEvents = events.filter(sift({
    $and: [
      { time: { $gt: 2000 } },
      { from: "bill" }
    ]
