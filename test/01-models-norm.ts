import { normalizeModels } from "../lib/engine/StoryConstants";
...
async function test() {
  ...
  expect(normalizeModels({ models: [] }, "openai/gpt-5"), [
    "openai/gpt-5",
    ...DEFAULT_LLM_SLUGS,
  ]);
  ...
  expect(normalizeModels({ models: ["deepseek/deepseek-r1"] }, ""), [
    "deepseek/deepseek-r1",
    ...DEFAULT_LLM_SLUGS,
  ]);
  ...
  const customDefaults: typeof DEFAULT_LLM_SLUGS = [
  ...
  expect(normalizeModels({ models: [] }, "openai/gpt-5", customDefaults), [
    "openai/gpt-5",
    ...customDefaults,
  ]);
